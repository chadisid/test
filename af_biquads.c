...
{
    av_log(NULL, AV_LOG_WARNING, "Inside config_filter \n");                   \
...
{
    av_log(NULL, AV_LOG_WARNING, "Inside config_output \n");
...
{
    av_log(NULL, AV_LOG_WARNING, "filter_channel \n");
    AVFilterLink *inlink = ctx->inputs[0];
    ThreadData *td = arg;
    AVFrame *buf = td->in;
    ...
    int ch;
    av_log(NULL, AV_LOG_WARNING, "filter_channel start %i end %i nb jobs %i jobnr %i bufchannels %i\n",start,end,nb_jobs,jobnr,buf->channels);
    for (ch = start; ch < end; ch++) {
        ...
        av_log(NULL, AV_LOG_WARNING, "filter_channel s->cache[ch].i1 %f s->cache[ch].i2 %f s->cache[ch].o1 %f s->cache[ch].o2 %f s->b0 %f s->b1 %f s->b2 %f s->a1 %f s->a2 %f s->cache[ch].clippings %i nb_samples %i disabled %i\n"
               ,s->cache[ch].i1,s->cache[ch].i2,s->cache[ch].o1,s->cache[ch].o2,s->b0,s->b1,s->b2,s->a1,s->a2,s->cache[ch].clippings,buf->nb_samples,ctx->is_disabled);
...
{
    av_log(NULL, AV_LOG_WARNING, "filter_frame \n");
...
{
    av_log(NULL, AV_LOG_WARNING, "process_command \n");
    AVFilterLink *outlink = ctx->outputs[0];
    int ret;
